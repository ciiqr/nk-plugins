...
# TODO: consider baking utilities like this nk... (ie. eval "$(nk plugin utilities bash)")
nk::run_for_output() {
    if [[ "$#" -lt 2 ]]; then
        echo 'usage: nk::run_for_output <output_var> <command> [<args>...]'
        return 1
    fi
    declare output_var="$1"
    declare -a command=("${@:2}")
    ...
    "${command[@]}" >"$__nk_tmp_output" 2>&1 || __nk_return_value="$?"
